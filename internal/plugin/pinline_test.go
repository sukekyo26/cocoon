...
func TestFormatPinLine_Exact(t *testing.T) {
	t.Parallel()
...
func TestFormatPinLine_Latest(t *testing.T) {
	t.Parallel()

	got := plugin.FormatPinLine("node", "latest")
	want := "node = \"latest\"\n"
	if got != want {
		t.Errorf("got:\n%s\nwant:\n%s", got, want)
	}
...
// FormatPinLine's scalar output byte-for-byte. This is the contract that
// lets the existing FormatPinLine call site keep working without a flag day.
func TestFormatPinLineWithExtras_NilEqualsFormatPinLine(t *testing.T) {
	t.Parallel()
	plain := plugin.FormatPinLine("go", "=1.23.4")
	withExtras := plugin.FormatPinLineWithExtras("go", "=1.23.4", nil)
	if plain != withExtras {
...
// emitted in alphabetical order under the inline-table form (with the
// constraint moved under the reserved "version" key) so the output is
// stable across calls (Go map iteration is randomised, which would
// otherwise drift the workspace.toml diff produced by pin --write).
func TestFormatPinLineWithExtras_SortedKeyOrder(t *testing.T) {
	...
	want := "android-sdk = { version = \"=14742923\", api_level = \"35\", build_tools = \"35.0.0\" }\n"
	if got != want {
		t.Errorf("got:\n%s\nwant:\n%s", got, want)
	}
}
...
func TestFormatPinLineWithExtras_QuotedValues(t *testing.T) {
	...
	want := "x = { version = \"=1.0\", label = \"quoted \\\"value\\\"\" }\n"
	if got != want {
		t.Errorf("got:\n%s\nwant:\n%s", got, want)
	}
}
...
// FormatPinLineWithExtras emits bare keys on output). When every extra is
// dropped the line collapses to the scalar form.
func TestFormatPinLineWithExtras_SkipsInvalidBareKeys(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want:   "x = { version = \"=1.0\", api_level = \"35\" }\n",
		},
		{
			name:   "key_with_space",
			extras: map[string]string{"weird key": "v", "api_level": "35"},
