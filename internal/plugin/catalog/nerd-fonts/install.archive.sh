...
#   CHECKSUM_ARM64   : unused — Meslo.tar.xz is architecture-independent, so
#                      only CHECKSUM_AMD64 is consulted regardless of arch
set -euo pipefail
...
if [ -n "$CHECKSUM_AMD64" ]; then
  echo "${CHECKSUM_AMD64}  /tmp/Meslo.tar.xz" | sha256sum -c -
else
