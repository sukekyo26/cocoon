// Package config loads, validates and re-emits the config file and plugin.toml.
//
// Optional sections are modelled as pointers so a missing section can be
// distinguished from an empty one.
package config

// Workspace mirrors the config file.
...
type LockFileSpec struct {
	// Name overrides the lock file's basename (written next to the discovered config file).
...
// means a reader can recreate the FROM line from the config file alone — no
...
type ContainerSpec struct {
	...
	// config file that still uses `os = "..."` / `os_version = "..."` (the
...
// in the FROM line — so users can read the config file and know which
...
var SupportedImageVersions = map[string][]string{
	...
	"rust":          {"1.96-bookworm", "1.96-trixie"},
	"denoland/deno": {"debian-2.8.2"}, // deno ships a single debian variant (no bookworm/trixie split).
}
...
// id (= cocoon catalog id); validation rejects config files that
...
// quotes, backslashes, whitespace — would let a repo-provided config file
...
//     .code-workspace file is written to (the config file's directory by
