...
passed.`,
	...
	"flag_lock_check_usage":     "verify the lock file matches workspace.toml without resolving (non-zero exit on drift; for CI)",
...
Subcommands:
  ...
  pin        print a workspace.toml [plugins].enable entry for one plugin
...
and the source layer it was read from.`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "Emit a [plugins].enable entry for workspace.toml (stdout, or in-place with --write)",
...
you to add to the [plugins].enable array in workspace.toml. A bare version
...
` + "`<id> = \"<method>\"`" + ` line for [plugins.methods].`,
	"flag_plugin_pin_method_usage": "install method name; only meaningful for plugins that declare [install.methods]. " +
		...
		"single workspace.toml read-write. When omitted, only the enable entry is updated " +
		"(the existing [plugins.methods] entry — or the plugin's default_method — stays in effect).",
	"flag_plugin_pin_write_usage": "upsert the enable entry in workspace.toml (auto-discovered from cwd)",
...
--upgrade を付けない限りネットワークアクセスなしで再利用されます。`,
	...
	"flag_lock_check_usage":     "解決せずに lock ファイルが workspace.toml と一致するか検証（ドリフト時は非ゼロ終了・CI 用）",
...
サブコマンド:
  ...
  pin        workspace.toml の [plugins].enable エントリを出力
...
source レイヤーを表示します。`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "workspace.toml 向けの [plugins].enable エントリを出力 (stdout、または --write で in-place)",
	"cmd_plugin_pin_long": `cocoon plugin pin — [plugins].enable エントリ (および任意で [plugins.methods]) を出力

既定では ` + "`\"<id>=<ref>\"`" + ` という配列要素が stdout に出力されるので、
workspace.toml の [plugins].enable 配列に追加してください。素のバージョン
...
両方をまとめて書き出します(または出力します)。`,
	"flag_plugin_pin_method_usage": "install メソッド名。[install.methods] を宣言したプラグインのみ意味を持ちます。" +
		"指定すると単一の workspace.toml 読み書きで [plugins.methods] と [plugins].enable エントリの両方を pin します。" +
		"省略時は enable エントリのみ更新されます " +
		"（既存の [plugins.methods] エントリ、またはプラグインの default_method が引き続き有効）。",
	"flag_plugin_pin_write_usage": "workspace.toml の enable エントリを upsert（cwd から自動探索）",
