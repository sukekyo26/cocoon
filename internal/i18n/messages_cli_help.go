...
…or open the project in VS Code and pick "Reopen in Container".`,
	"flag_gen_workspace_usage": "path to cocoon.toml (default: discovered from cwd)",
	"flag_gen_output_usage":    "project root to write generated artifacts under (default: directory of cocoon.toml)",
...
passed.`,
	"flag_lock_workspace_usage": "path to cocoon.toml (default: discovered from cwd)",
	"flag_lock_check_usage":     "verify the lock file matches cocoon.toml without resolving (non-zero exit on drift; for CI)",
...
Examples:
  ...
  cocoon gen workspace --folder ~/.claude=Claude --name my-stack`,
	"flag_gen_workspace_workspace_usage": "path to cocoon.toml (default: discovered from cwd)",
	"flag_gen_workspace_output_usage":    "project root to write under (default: directory of cocoon.toml)",
...
Subcommands:
  ...
  pin        print a cocoon.toml [plugins].enable entry for one plugin
  scaffold   create a new <id>/ directory from a template

To use a plugin, add its id to [plugins].enable in cocoon.toml — the
...
and the source layer it was read from.`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "Emit a [plugins].enable entry for cocoon.toml (stdout, or in-place with --write)",
...
you to add to the [plugins].enable array in cocoon.toml. A bare version
...
` + "`<id> = \"<method>\"`" + ` line for [plugins.methods].`,
	"flag_plugin_pin_method_usage": "install method name; only meaningful for plugins that declare [install.methods]. " +
		...
		"single cocoon.toml read-write. When omitted, only the enable entry is updated " +
		"(the existing [plugins.methods] entry — or the plugin's default_method — stays in effect).",
	"flag_plugin_pin_write_usage": "upsert the enable entry in cocoon.toml (auto-discovered from cwd)",
...
auto-discovered from the nearest cocoon.toml. Pass --plugins-dir <path> to
...
emitted (kept plugin-scoped, not per-method).`,
	"flag_plugin_scaffold_cmd_plugins_dir_usage":       "output directory (default: <workspace>/.cocoon/plugins, auto-discovered from cocoon.toml)",
...
…または VS Code でプロジェクトを開き「Reopen in Container」を選びます。`,
	"flag_gen_workspace_usage": "cocoon.toml のパス（既定: cwd から探索）",
	"flag_gen_output_usage":    "生成物の出力先プロジェクトルート（既定: cocoon.toml のあるディレクトリ）",
...
--upgrade を付けない限りネットワークアクセスなしで再利用されます。`,
	"flag_lock_workspace_usage": "cocoon.toml のパス（既定: cwd から探索）",
	"flag_lock_check_usage":     "解決せずに lock ファイルが cocoon.toml と一致するか検証（ドリフト時は非ゼロ終了・CI 用）",
...
例:
  ...
  cocoon gen workspace --folder ~/.config/nvim
  cocoon gen workspace --folder ~/.claude=Claude --name my-stack`,
	"flag_gen_workspace_workspace_usage": "cocoon.toml のパス（既定: cwd から探索）",
	"flag_gen_workspace_output_usage":    "出力先（既定: cocoon.toml のあるディレクトリ）",
...
サブコマンド:
  ...
  pin        cocoon.toml の [plugins].enable エントリを出力
  scaffold   テンプレートから新しい <id>/ ディレクトリを作成

プラグインを使うには cocoon.toml の [plugins].enable に id を追加します。
...
source レイヤーを表示します。`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "cocoon.toml 向けの [plugins].enable エントリを出力 (stdout、または --write で in-place)",
	"cmd_plugin_pin_long": `cocoon plugin pin — [plugins].enable エントリ (および任意で [plugins.methods]) を出力

既定では ` + "`\"<id>=<ref>\"`" + ` という配列要素が stdout に出力されるので、
cocoon.toml の [plugins].enable 配列に追加してください。素のバージョン
...
両方をまとめて書き出します(または出力します)。`,
	"flag_plugin_pin_method_usage": "install メソッド名。[install.methods] を宣言したプラグインのみ意味を持ちます。" +
		"指定すると単一の cocoon.toml 読み書きで [plugins.methods] と [plugins].enable エントリの両方を pin します。" +
		"省略時は enable エントリのみ更新されます " +
		"（既存の [plugins.methods] エントリ、またはプラグインの default_method が引き続き有効）。",
	"flag_plugin_pin_write_usage": "cocoon.toml の enable エントリを upsert（cwd から自動探索）",
...
（最寄りの cocoon.toml から自動探索）。--plugins-dir <path> で上書きすると、
...
スコープ・メソッド非依存) も生成されます。`,
	"flag_plugin_scaffold_cmd_plugins_dir_usage":       "出力先ディレクトリ（既定: cocoon.toml から自動探索した <workspace>/.cocoon/plugins）",
