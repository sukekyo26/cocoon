...
that repository.`,
	...
	// cocoon init
	"cmd_init_short": "Create cocoon.toml in the current directory",
	"cmd_init_long": `cocoon init — generate cocoon.toml in the current directory
...
to drive non-interactively from CI.`,
	...
	"flag_init_mount_root_usage":      `mount range: "." (cwd, default), ".." (parent), or a deeper ".." chain (e.g. "../..")`,
	...
	"flag_init_sudo_usage":            "in-container sudo policy — one of: %s. nopasswd: passwordless sudo (default); password: sudo requires SUDO_PASSWORD from .devcontainer/.env.local (build secret); none: no_new_privileges = true (disables sudo)",
...
directory (walking parent directories until a .git boundary or $HOME),
assembles the layered
...
…or open the project in VS Code and pick "Reopen in Container".`,
	"flag_gen_workspace_usage": "path to the config file (default: discovered from cwd)",
	"flag_gen_output_usage":    "project root to write generated artifacts under (default: the config file's directory)",
	"flag_gen_locked_usage":    `fail if any plugin uses "latest" without a lock-file entry (reproducible CI)`,

	// cocoon lock
	"cmd_lock_short": "Resolve plugin versions and write the lock file for reproducible builds",
...
the lock file (cocoon.lock by default; override via [lockfile].name) at the
workspace root. ` + "`cocoon gen`" + ` then consumes the lock
...
passed.`,
	"flag_lock_workspace_usage": "path to the config file (default: discovered from cwd)",
	"flag_lock_check_usage":     "verify the lock file matches the config file without resolving (non-zero exit on drift; for CI)",
...
Examples:
  ...
  cocoon gen workspace --folder ~/.claude=Claude --name my-stack`,
	"flag_gen_workspace_workspace_usage": "path to the config file (default: discovered from cwd)",
	"flag_gen_workspace_output_usage":    "project root to write under (default: the config file's directory)",
...
Subcommands:
  ...
  pin        print a [plugins].enable entry for one plugin
  scaffold   create a new <id>/ directory from a template

To use a plugin, add its id to [plugins].enable in your config file — the
...
and the source layer it was read from.`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "Emit a [plugins].enable entry for your config file (stdout, or in-place with --write)",
	"cmd_plugin_pin_long": `cocoon plugin pin — emit an enable entry for [plugins].enable (and optionally [plugins.methods])

By default a ` + "`\"<id>=<ref>\"`" + ` array element is printed to stdout for
you to add to the [plugins].enable array in your config file. A bare version
(1.23.4) is written as an exact pin (<id>=1.23.4); pass "latest" to track
the newest release. With --write the entry is upserted in place (added, or
the existing <id> element is replaced); the rest of the file is preserved
verbatim.

Versions are frozen to concrete releases — and per-arch SHA256 checksums
recorded — by ` + "`cocoon lock`" + ` into the lock file; checksums no longer
...
both: the ` + "`\"<id>=<ref>\"`" + ` enable entry and a
` + "`<id> = \"<method>\"`" + ` line for [plugins.methods].`,
	"flag_plugin_pin_method_usage": "install method name; only meaningful for plugins that declare [install.methods]. " +
		"When set, the command pins both [plugins.methods] and the [plugins].enable entry in a " +
		"single config-file read-write. When omitted, only the enable entry is updated " +
		"(the existing [plugins.methods] entry — or the plugin's default_method — stays in effect).",
	"flag_plugin_pin_write_usage": "upsert the enable entry in the config file (auto-discovered from cwd)",
...
auto-discovered from the nearest config file. Pass --plugins-dir <path> to
...
emitted (kept plugin-scoped, not per-method).`,
	"flag_plugin_scaffold_cmd_plugins_dir_usage":       "output directory (default: <workspace>/.cocoon/plugins, auto-discovered from the config file)",
...
そのリポジトリ専用の Dev Container または docker-compose スタックを生成します。`,
	...
	// cocoon init
	"cmd_init_short": "カレントディレクトリに cocoon.toml を生成",
	"cmd_init_long": `cocoon init — カレントディレクトリに cocoon.toml を生成
...
--plugins / --alias-bundles を渡します。`,
	...
	"flag_init_mount_root_usage":      `マウント範囲: "."（cwd、既定）/ ".."（親）/ より深い ".." 連結（例 "../.."）`,
	...
	"flag_init_sudo_usage":            "コンテナ内 sudo の方針 — %s のいずれか。nopasswd: パスワード不要 sudo（既定）/ password: .devcontainer/.env.local の SUDO_PASSWORD を要求（build secret）/ none: no_new_privileges = true（sudo 無効化）",
...
…または VS Code でプロジェクトを開き「Reopen in Container」を選びます。`,
	"flag_gen_workspace_usage": "設定ファイルのパス（既定: cwd から探索）",
	"flag_gen_output_usage":    "生成物の出力先プロジェクトルート（既定: 設定ファイルのあるディレクトリ）",
	"flag_gen_locked_usage":    `lock ファイルのエントリ無しで "latest" を使うプラグインがあれば失敗（再現性 CI 用）`,

	// cocoon lock
	"cmd_lock_short": "プラグインのバージョンを解決し lock ファイルを書き出して再現性を確保",
	"cmd_lock_long": `cocoon lock — 有効な version_capable プラグインの制約を、ネットワーク越しに
具体バージョン（と arch ごとの SHA256 checksum）へ解決し、workspace ルートに
lock ファイル（既定 cocoon.lock、[lockfile].name で上書き可）を書き出します。` + "`cocoon gen`" + ` はこの lock をオフラインで消費し、
...
--upgrade を付けない限りネットワークアクセスなしで再利用されます。`,
	"flag_lock_workspace_usage": "設定ファイルのパス（既定: cwd から探索）",
	"flag_lock_check_usage":     "解決せずに lock ファイルが設定ファイルと一致するか検証（ドリフト時は非ゼロ終了・CI 用）",
...
例:
  ...
  cocoon gen workspace --folder ~/.config/nvim
  cocoon gen workspace --folder ~/.claude=Claude --name my-stack`,
	"flag_gen_workspace_workspace_usage": "設定ファイルのパス（既定: cwd から探索）",
	"flag_gen_workspace_output_usage":    "出力先（既定: 設定ファイルのあるディレクトリ）",
...
サブコマンド:
  ...
  pin        [plugins].enable エントリを出力
  scaffold   テンプレートから新しい <id>/ ディレクトリを作成

プラグインを使うには設定ファイルの [plugins].enable に id を追加します。
...
source レイヤーを表示します。`,
	...
	// cocoon plugin pin
	"cmd_plugin_pin_short": "設定ファイル向けの [plugins].enable エントリを出力 (stdout、または --write で in-place)",
	"cmd_plugin_pin_long": `cocoon plugin pin — [plugins].enable エントリ (および任意で [plugins.methods]) を出力

既定では ` + "`\"<id>=<ref>\"`" + ` という配列要素が stdout に出力されるので、
設定ファイルの [plugins].enable 配列に追加してください。素のバージョン
(1.23.4) は exact pin (<id>=1.23.4) として書かれ、"latest" を渡すと最新リリースを
追従します。--write を付けると in-place で upsert されます (新規追加、または既存の
<id> 要素を置換)。ファイルの残りは verbatim で保持されます。

バージョンの具体リリースへの凍結と arch ごとの SHA256 checksum の記録は
` + "`cocoon lock`" + ` が lock ファイルに対して行います。checksum はもう
設定ファイルには置きません。

plugin.toml の [install.methods] に複数エントリを宣言しているプラグインには
--method <name> を指定します。pin はその場合に ` + "`\"<id>=<ref>\"`" + ` の
enable エントリと ` + "`<id> = \"<method>\"`" + ` 行（[plugins.methods] 向け）の
両方をまとめて書き出します(または出力します)。`,
	"flag_plugin_pin_method_usage": "install メソッド名。[install.methods] を宣言したプラグインのみ意味を持ちます。" +
		"指定すると単一の設定ファイル読み書きで [plugins.methods] と [plugins].enable エントリの両方を pin します。" +
		"省略時は enable エントリのみ更新されます " +
		"（既存の [plugins.methods] エントリ、またはプラグインの default_method が引き続き有効）。",
	"flag_plugin_pin_write_usage": "設定ファイルの enable エントリを upsert（cwd から自動探索）",
...
（最寄りの設定ファイルから自動探索）。--plugins-dir <path> で上書きすると、
...
スコープ・メソッド非依存) も生成されます。`,
	"flag_plugin_scaffold_cmd_plugins_dir_usage":       "出力先ディレクトリ（既定: 設定ファイルから自動探索した <workspace>/.cocoon/plugins）",
