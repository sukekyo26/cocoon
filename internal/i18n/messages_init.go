...
var messagesEN_init = map[string]string{
	...
	"init_desc_image_version_static":    "Type a tag, or press Tab to cycle through curated suggestions (e.g. 1.26-bookworm). Any well-formed tag the upstream registry publishes is accepted. Rules: first character is a letter, digit, or underscore; trailing characters add `.` and `-`; no slash, no colon.",
	...
	"init_desc_mount_root":              "How much of your filesystem should be visible inside the container? Pick Custom to reach an ancestor more than one level up.",
	"init_option_mount_cwd":             "Just this project (.)",
	"init_option_mount_parent":          "Parent directory — sibling repos visible (..)",
	"init_option_mount_custom":          "Custom — type a `..` chain (e.g. ../..)",
	"init_prompt_mount_root_custom":     "Mount range — how many levels up?",
	"init_desc_mount_root_custom":       "Type a chain of `..` segments to mount an ancestor N levels above this project (e.g. \"../..\" = grandparent, \"../../..\" = three levels up). Only \".\" or a pure `..` chain is accepted.",
	"init_err_mount_root_fmt":           "mount_root must be \".\" or a chain of \"..\" segments (e.g. \"..\", \"../..\")",
	"init_prompt_dir":                   "Container workdir name (under /home/<user>/, leave blank for \"workspace\")",
	"init_desc_dir":                     "Where the project lands inside the container as /home/<user>/<dir>[/<service>]. Default \"workspace\". Slashes allowed for nested paths (e.g. \"work/myproject\"). Useful when a tool like AWS SAM expects the in-container path to mirror a specific host layout.",
	"init_err_dir_fmt":                  "must be one or more path segments of [A-Za-z0-9._-] joined by `/` — no leading/trailing slash, no `.`/`..` segments",
	...
	"init_desc_plugin_method":           "This plugin declares more than one install method (e.g. official installer vs. direct binary). Pick the one that fits your environment — the choice is saved to [plugins.methods] in cocoon.toml.",
	...
	// cocoon.toml inline comments (rendered into the generated file)
	"init_toml_header": "# cocoon.toml — cocoon configuration (generated by `cocoon init`)\n# Edit freely; re-run `cocoon gen` to regenerate .devcontainer/.\n",
	"init_toml_section_workspace": "# [workspace] — generation-wide knobs.\n" +
		"#   mount_root: how much of your filesystem to expose. \".\" = cwd only, \"..\" = parent (siblings visible), \"../..\" = further ancestors.\n" +
...
var messagesJA_init = map[string]string{
	...
	"init_desc_image_version_static":    "タグを入力するか、Tab キーで推奨候補を循環できます (例: 1.26-bookworm)。上流レジストリが公開している正しい形式のタグなら自由に入力可。形式: 先頭は英数字または `_`、2 文字目以降は `.` / `-` も可、スラッシュ・コロン禁止。",
	...
	"init_desc_mount_root":              "コンテナ内に見せるファイルシステムの範囲を選んでください。2 段以上上の祖先を見せたい場合は「カスタム」を選択。",
	"init_option_mount_cwd":             "このプロジェクトのみ (.)",
	"init_option_mount_parent":          "親ディレクトリ — 兄弟リポジトリも見える (..)",
	"init_option_mount_custom":          "カスタム — `..` チェーンを入力 (例: ../..)",
	"init_prompt_mount_root_custom":     "マウント範囲 — 何段上にしますか？",
	"init_desc_mount_root_custom":       "`..` を連結して、このプロジェクトから N 段上の祖先をマウントします (例: \"../..\" = 2 段上、\"../../..\" = 3 段上)。受け付けるのは \".\" か `..` のみの連結だけです。",
	"init_err_mount_root_fmt":           "mount_root は \".\" または \"..\" の連結 (例: \"..\", \"../..\") を指定してください",
	"init_prompt_dir":                   "コンテナ内 workdir 名 (/home/<user>/ 配下、空 Enter で \"workspace\")",
	"init_desc_dir":                     "プロジェクトがコンテナ内の /home/<user>/<dir>[/<service>] にマウントされます。デフォルトは \"workspace\"。スラッシュで多段階層も可 (例 \"work/myproject\")。AWS SAM などコンテナ内パスをホスト構成に合わせたいツール向け。",
	"init_err_dir_fmt":                  "[A-Za-z0-9._-] の path セグメントを `/` で連結した形式である必要があります (先頭/末尾スラッシュ・`.`/`..` セグメント禁止)",
	...
	"init_desc_plugin_method":           "このプラグインは複数のインストール方式（例: 公式インストーラ / バイナリ直接ダウンロード）を提供しています。環境に合うものを 1 つ選択してください — 選択は cocoon.toml の [plugins.methods] に保存されます。",
	...
	// cocoon.toml inline comments (rendered into the generated file)
	"init_toml_header": "# cocoon.toml — cocoon 設定 (cocoon init で生成)\n# 自由に編集してください。cocoon gen で .devcontainer/ を再生成します。\n",
	"init_toml_section_workspace": "# [workspace] — 生成全体の挙動。\n" +
		"#   mount_root: コンテナへ見せるホスト範囲。\".\" = cwd のみ、\"..\" = 親ディレクトリ（兄弟リポも見える）、\"../..\" = さらに上の祖先。\n" +
		"#   dir: コンテナ内 workdir の親 (/home/<user>/ 配下)。デフォルト \"workspace\"。スラッシュで多段階層も可。\n" +
	...
	"init_toml_section_container_shell": "# [container.shell] — ログインシェル + シェル別 rc 注入。\n" +
		...
		"#   env の値は rc を source する時にシェルが展開する。$HOME / $PATH をそのまま書ける\n" +
		"#   (例: NPM_CONFIG_PREFIX = \"$HOME/.local\")。$(cmd) コマンド置換は bash/zsh では常に、\n" +
		"#   fish では 3.4+ で動く (それ以前は fish 固有の (cmd) 記法)。リテラルの $ は値の中で\n" +
		"#   \\$ にする — TOML では通常文字列 \"\\\\$RAW\" かリテラル文字列 '\\$RAW' と書く。\n" +
		"#   alias の本文は呼び出し時まで展開されない。\n" +
		"#\n" +
