...
var messagesEN_init = map[string]string{
	...
	"gen_unlocked_latest_warning":           "WARNING: plugin %q uses \"latest\" without a cocoon.lock entry; the build will resolve it non-reproducibly. Run `cocoon lock` to freeze it (or `cocoon gen --locked` to make this an error).",
...
var messagesJA_init = map[string]string{
	...
	"gen_unlocked_latest_warning":           "警告: プラグイン %q は cocoon.lock のエントリ無しで \"latest\" を使っています。build 時に非再現的に解決されます。`cocoon lock` で凍結してください（`cocoon gen --locked` でエラー化できます）。",
