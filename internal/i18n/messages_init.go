...
var messagesEN_init = map[string]string{
	...
	"init_prompt_plugin_version":        "%s — pin a specific version?",
	"init_desc_plugin_version":          "Select No to let cocoon resolve the latest at container build time. Select Yes to enter a pin string.",
	"init_plugin_version_pin_label":     "Yes (pin)",
	"init_plugin_version_latest_label":  "No (LATEST)",
	"init_prompt_plugin_version_pin":    "%s version",
	"init_desc_plugin_version_pin":      "Version string the plugin's install.sh should fetch (e.g. 1.23.4, 0.5.7). First character alnum or underscore; trailing characters may add `.` and `-`; no slash or colon.",
...
var messagesJA_init = map[string]string{
	...
	"init_prompt_plugin_version":        "%s — バージョンを固定しますか？",
	"init_desc_plugin_version":          "No を選ぶとコンテナビルド時に最新版を自動取得します。Yes を選ぶと特定バージョンを入力できます。",
	"init_plugin_version_pin_label":     "はい (固定)",
	"init_plugin_version_latest_label":  "いいえ (LATEST)",
	"init_prompt_plugin_version_pin":    "%s のバージョン",
	"init_desc_plugin_version_pin":      "プラグインの install.sh が取得するバージョン文字列を入力 (例: 1.23.4, 0.5.7)。先頭は英数字または `_`、2 文字目以降は `.` / `-` も可。スラッシュ・コロン禁止。",
