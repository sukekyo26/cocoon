...
// name from the config file contains characters that cannot safely be
...
// WorkspaceContext is a normalized read-only view over the config file that
...
type WorkspaceContext struct {
	...
	PluginsFS fs.FS
	// ProjectDir holds the directory containing the discovered config file. envfile uses
...
// stricter regex for config-file input; this check is the
