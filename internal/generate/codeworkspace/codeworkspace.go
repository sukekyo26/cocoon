...
// the [code_workspace] section of cocoon.toml.
//
...
// cocoon.toml. VS Code resolves the result from wherever the
...
var (
	...
	// ErrNoFolders signals that neither cocoon.toml nor the caller
...
// cocoon.toml folders so CLI --folder flags supplement the declarative
...
// ctx.ProjectDir (the common "next to cocoon.toml" case).
type Options struct {
...
// collectFolders concatenates the cocoon.toml folders with any
...
// "../sibling" stays semantically tied to where cocoon.toml lives), then
