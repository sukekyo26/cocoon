...
`.env.local` is the one host-local exception to "init writes only `workspace.toml`": in `[container.sudo]` password mode it holds the gitignored `SUDO_PASSWORD` secret, seeded by `cocoon init --sudo password` (interactively) and consumed at build time as a Docker build secret. It is never committed and never regenerated by `cocoon gen`.
