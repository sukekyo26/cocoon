...
Read the config file, resolve the layered plugin catalog (project ∪ user ∪ embedded), and write `.devcontainer/`. Plugin install scripts are inlined directly into the generated Dockerfile, so the build needs no external context beyond the project tree. Generation is fully offline: when a [`cocoon.lock`](#cocoon-lock) is present, each locked plugin's resolved version and per-arch checksums are baked into the Dockerfile (`PIN` / `CHECKSUM_*`) so the build is reproducible.

### Flags

| Flag | Type | Description |
|---|---|---|
| `--workspace <path>` | string | Path to the config file (default: discovered from cwd). |
| `--output <dir>` | string | Project root to write artifacts under (default: directory of the config file). |
...
|---|---|---|
| `--workspace <path>` | string | Path to the config file (default: discovered from cwd). |
| `--output <dir>` | string | Project root to write the `.code-workspace` under (default: directory of the config file). |
...
```bash
# Use [code_workspace] from the config file as-is
cocoon gen workspace

# Add ad-hoc folders without editing the config file
...
Resolve every enabled `version_capable` plugin's `[plugins].enable` version pin to a concrete version (plus per-arch SHA256 checksums) over the network, and write `cocoon.lock` at the workspace root (next to the config file). `cocoon gen` then consumes `cocoon.lock` offline so the generated `.devcontainer/` is reproducible — same plugin versions, same checksums, no network call at generation time.
...
- The lock file is named `cocoon.lock` by default; set [`[lockfile].name`](configuration.md#lockfile) in the config file to use a different basename (both `cocoon lock` and `cocoon gen` honor it).
...
|---|---|---|
| `--workspace <path>` | string | Path to the config file (default: discovered from cwd). |
| `--check` | bool | Verify `cocoon.lock` matches the config file **without resolving** (no network). Exits non-zero on drift — a missing lock, a changed `inputs_hash`, or any enabled plugin whose recorded `requested` no longer matches. For CI. |
...
# CI gate: fail if the committed lock no longer matches the config file
cocoon lock --check
```
...
| `--write` | Upsert the pin element in the config file's `[plugins].enable` array (auto-discovered from cwd). |
...
| Flag | Description |
|---|---|
| `--plugins-dir <path>` | Output directory. Default: `<workspace>/.cocoon/plugins` (auto-discovered from the config file). |
