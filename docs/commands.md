...
| `--image-version <ver>` | string | Base image tag. Any well-formed Docker tag (letters, digits, dot, underscore, hyphen — no slash, no colon) is accepted; existence in the upstream registry is left to `docker pull`. Requires `--image` to be set. |
