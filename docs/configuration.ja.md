...
ログインシェルとシェル別 rc 注入。aliases / env はイメージビルド時にコンテナ内 rc ファイルへ追記されます。bash と zsh は POSIX 記法 (`alias k='v'`、`export K=V`) を共有、fish は自動翻訳。env の値はダブルクォートで出力するため、rc を source した時点で `$HOME` / `$PATH` / `$(cmd)` がシェルにより展開されます（例: `NPM_CONFIG_PREFIX = "$HOME/.local"` は `/home/<user>/.local` になります）。alias の本文はシングルクォートのまま出力します。alias は呼び出し時にシェルが本文を再パースするので、`$1` や `$HOME` を含めても呼び出し時に正しく解釈されます。
