...
読み込みます。いずれも `cocoon.toml` と同じ場所です。

| フィールド | 型 | デフォルト | 説明 |
|---|---|---|---|
| `name` | string | `cocoon.lock` | lock ファイルの basename。`[A-Za-z0-9._-]` の単一ファイル名のみ — `/` 不可（lock は常に `cocoon.toml` の隣に置かれる）、`.` / `..` 不可、`cocoon.toml` 不可（設定ファイルを上書きするため）。 |
