...
という catalog 側のポリシー。per-arch checksum は `workspace.toml` ではなく
...
| `PIN`            | per-RUN env、`[version].version_capable = true` のときのみ | `workspace.toml` の `[plugins].enable` 配列の `<id>=<version>` 要素の値（例: `"go=1.23.4"` → `PIN="1.23.4"`）。未 pin の id や lock 未生成の `latest` 要素では空で、upstream 最新を使う |
...
| `<EXTRA_ENV>`    | per-RUN env、`[install.extra_versions]` で宣言されたときのみ | 宣言された subcomponent 版ごとに 1 つの env が出る。プラグイン側で env 名と default を宣言し、ユーザーは `workspace.toml` の `[plugins.options].<id>` インラインテーブルに同じキー名を書いて値を上書きできる（例: `android-sdk = { api_level = "36" }`。主バージョンは `[plugins].enable` に `"android-sdk=..."` として別途 pin）。予約 env 名（上記）と衝突する env 名は宣言できない。詳細は後述の「サブコンポーネントバージョン」節を参照 |
...
ユーザー側 workspace.toml では `[plugins.options]` インラインテーブルの
