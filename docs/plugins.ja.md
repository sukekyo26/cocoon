...
| `CHECKSUM_AMD64` | per-RUN env、`version_capable = true` かつ `verify = "checksum"` のときのみ | amd64 アーティファクトの `sha256`。空ならスクリプトは上流公開 checksum で検証する（上流が公開していない場合のみ警告）。`verify = "pgp"` プラグインには渡されない |
