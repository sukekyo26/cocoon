...
`plugin.toml` のスキーマ、`install.<name>.sh` / `install_user.sh` の使い分け、
スクリプトに渡される環境変数、バージョン pin の契約を網羅する。

既存プラグインを単に有効化したいだけなら、`cocoon.toml` の
...
- `install.<name>.sh` は `docker build` 中に実行される。`[install].requires_root`
  に応じて root または非特権ユーザーで動く。
- `install_user.sh` は **常に非特権ユーザー** として `install.<name>.sh` の後に
  実行される。root インストールとユーザー設定を分離する必要がある
  特殊ケースのためだけに存在する（後述の「`install.<name>.sh` と `install_user.sh` の使い分け」を参照）。
...
（例: `gh-cli`、`bun-sh`）は cocoon.toml の `[plugins.methods]` を
...
| `[install]`  | `default_method`  | string             | —     | ✓ | `cocoon.toml` の `[plugins.methods]` で指定が無いときに採用される method 名。`[install.methods]` のキー名と一致する必要がある。`[install.methods]` 必須化のためこのフィールドも必須 (method 1 つだけのプラグインも 1 エントリ宣言する) |
...
| `[install.extra_versions.<key>]` | `env`, `default` | inline table | — |   | ユーザーが上書き可能なサブコンポーネント版を宣言する。`<key>` は `^[a-z][a-z0-9_]*$`；`env` は install スクリプトに渡る変数名（`^[A-Z_][A-Z0-9_]*$`、予約 env 名や `build_args` と衝突不可）；`default` は `[plugins.options].<id>` で key が未指定のときに使われる値。詳細は「サブコンポーネントバージョン」節を参照 |
...
## `install.<name>.sh` と `install_user.sh` の使い分け

ほぼ全プラグインは `install.<name>.sh` だけで済む。`install_user.sh` を
書くべきは次の **両方** が成立する場合に限る:

1. `[install].requires_root = true`（`install.<name>.sh` が root で動く）かつ
...
| 状況 | 選択 |
|---|---|
| `requires_root = false`、rc 編集なし | `install.<name>.sh` のみ |
| `requires_root = false`、rc 編集あり | `install.<name>.sh` のみ — 中で `$RC_FILE` に書く |
| `requires_root = true`、`apt-get` / `/usr/local/bin` のみ | `install.<name>.sh` のみ |
| `requires_root = true`、rc 編集 / `~/.config/<tool>` 書込 / `<tool> init` | `install.<name>.sh` **と** `install_user.sh` |

### 具体例

| プラグイン | `install.<name>.sh` の役割 | `install_user.sh` の役割 |
|---|---|---|
...
インストール経路が選べるようになる。選択は `cocoon.toml` に保存される:
...
`cocoon.toml` 側の旧参照が残っていても誤ったアーティファクトを引っ張る前
...
**pin / checksum のスコープ。** バージョン pin は `[plugins].enable` 配列に
インライン（例: `"go=1.23.4"`）で書かれ、**プラグイン単位（method 別ではない）**
で保持される — catalog の `plugin.toml` を method 別 checksum で肥大化させない
という catalog 側のポリシー。per-arch checksum は `cocoon.toml` ではなく
`cocoon lock` が `cocoon.lock` に記録する。method を切り替えるときは `cocoon lock`
を再実行して記録済み checksum を更新する。さもないと install スクリプトの
`sha256sum -c -` が新しいアーティファクトと合わず落ちる。
...
`install.<name>.sh` も `install_user.sh` も `bash <<'COCOON_PLUGIN_EOF' … COCOON_PLUGIN_EOF`
...
| `PIN`            | per-RUN env、`[version].version_capable = true` のときのみ | `cocoon.toml` の `[plugins].enable` 配列の `<id>=<version>` 要素の値（例: `"go=1.23.4"` → `PIN="1.23.4"`）。未 pin の id や lock 未生成の `latest` 要素では空で、upstream 最新を使う |
...
| `<EXTRA_ENV>`    | per-RUN env、`[install.extra_versions]` で宣言されたときのみ | 宣言された subcomponent 版ごとに 1 つの env が出る。プラグイン側で env 名と default を宣言し、ユーザーは `cocoon.toml` の `[plugins.options].<id>` インラインテーブルに同じキー名を書いて値を上書きできる（例: `android-sdk = { api_level = "36" }`。主バージョンは `[plugins].enable` に `"android-sdk=..."` として別途 pin）。予約 env 名（上記）と衝突する env 名は宣言できない。詳細は後述の「サブコンポーネントバージョン」節を参照 |

ホスト側での評価は一切行われない — bash はビルド環境内で本体を実行し、
その環境変数は上記 2 ソースから組み立てられる。

リテラル文字列 `COCOON_PLUGIN_EOF` を `install.<name>.sh` / `install_user.sh`
...
- ダウンロードを検証する。方式は `[version].verify` で選ぶ:
  - `verify = "checksum"`（既定）— ユーザーが checksum を pin した場合
    （amd64 なら `$CHECKSUM_AMD64`、arm64 なら `$CHECKSUM_ARM64`。アーキ
    非依存アセットはどちらを参照してもよい）はそれで検証する。pin が無い
  ...
  - `verify = "pgp"` — SHA256 を公開せず detached 署名のみ配布する
    ...
    `$CHECKSUM_ARM64` は渡されず、その種のプラグインは checksum を
    一切持たない。

ユーザー側は `[plugins].enable` 配列にインラインでバージョンを pin する —
バージョンは id と同じ要素の `=` の直後に書く。checksum はここには記録せず、
`cocoon.lock` に存在する（`cocoon lock` 経由）:
...
`cocoon plugin pin <id> <ref> --write` がこの要素を upsert する。
フラグ詳細は `docs/commands.ja.md` 参照。

`version_capable = false` のプラグインは pin できない。`cocoon gen` は
そのプラグインの `<id>=<version>` 要素を拒否し、`cocoon plugin pin`
もエントリを出力しない。
...
- **キー名**（`api_level`, `build_tools`）— ユーザーが
  `[plugins.options].<id>` 内に書く名前。`^[a-z][a-z0-9_]*$` に一致。
  `version` は extra キーとしては拒否される — プラグイン自身のバージョンは
  `[plugins.options]` ではなく `[plugins].enable` 配列で pin するため、
  ここでの `version` キーは no-op（ユーザーが上書きできない）になる。
- **`env`** — install スクリプトが読む環境変数名。`^[A-Z_][A-Z0-9_]*$`、
  予約 env 名（前述）や `[install].build_args` の名前、`extra_versions`
  内の他の `env` と衝突不可。
- **`default`** — 必須・非空。`cocoon.toml` で未指定時に使われる値。
...
ユーザー側 cocoon.toml では `[plugins.options]` インラインテーブルの
サイドテーブルで上書きし、プラグイン自身のバージョンは `[plugins].enable`
配列に書く:

```toml
[plugins]
enable = [
    "android-sdk=14742923",   # cmdline-tools のバージョン（→ $PIN）
]
...
- **`ErrHeredocCollision: plugin "x" contains the literal
  COCOON_PLUGIN_EOF`** — `install.<name>.sh` 内で heredoc terminator と
...
- [`architecture.ja.md`](architecture.ja.md) — LayeredFS と inline-heredoc
  設計の why。
- [`configuration.ja.md`](configuration.ja.md) — `cocoon.toml` の
  `[plugins]` / `[plugins.methods]` / `[plugins.options]` セクション。
