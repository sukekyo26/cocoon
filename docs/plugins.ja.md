...
plugins/<id>/
├── plugin.toml         # 必須
├── install.sh          # 単一 method 形式（legacy） — 下記参照
├── install.<name>.sh   # method 形式（[install.methods.<name>] ごとに 1 ファイル）
└── install_user.sh     # 任意（「install.sh と install_user.sh の使い分け」のケースのみ）
```

プラグインは次のどちらか **片方** の形になる:

- **単一 method (legacy)**: `install.sh` を持つ。`plugin.toml` に
  `[install.methods]` セクションは無い。v0.4 以前は全プラグインがこの形で、
  今もほとんどのプラグインはこの形のまま。
- **複数 method**: `plugin.toml` に `[install.methods.<name>]` を宣言し、
  各 `<name>` に対応する `install.<name>.sh` を配置する。`install.sh` は
  ロード時に拒否される — 両方混在は許容されない。同じツールに対して
  代替インストール経路が必要な場合（例: `gh.io` インストーラ vs.
  `gh.io` を遮断する Zscaler 環境向けの直接バイナリ配布、あるいは
  `curl|sh` を社内ポリシーが禁ずる場合）に選ぶ。

プラグインの **install snippet**（生成 Dockerfile 上の
`# Install …` コメント + RUN ブロック）が出力されるのは、次のいずれかが
存在するときだけ:

- `install.sh`（単一 method）または `install.<name>.sh`（複数 method）
- `install_user.sh`
- `[install.env]`
...
| `[install]`  | `default_method`  | string             | —     | `[install.methods]` を宣言する場合は ✓ | `workspace.toml` の `[plugins.methods]` で指定が無いときに採用される method 名。`[install.methods]` のキー名と一致する必要があり、`[install.methods]` 未宣言時には設定禁止 |
| `[install.methods.<name>]` | `description` | string | — | method を宣言したら ✓ | `cocoon init` の method ピッカーに表示される一行説明。`<name>` は `^[a-z][a-z0-9_-]*$` に一致し、対応する `install.<name>.sh` がディスクに存在しなければならない |
...
## 複数 method プラグイン（`[install.methods]`）

`plugin.toml` に `[install.methods.<name>]` を 2 つ以上宣言し、各
`<name>` に対応する `install.<name>.sh` をディスクに配置すると、その
プラグインは複数のインストール経路を提供できる。`install.sh` は禁止
（loader が拒否）。
...
対応する `install.gh-cli.sh` / `install.binary.sh` を置けば、2 つの
...
`cocoon init` は宣言された method が 2 つ以上あるプラグインに対してのみ
per-plugin の method ピッカーを表示し、選択結果を `[plugins.methods]` に
書き出す。ピッカーの初期選択行はプラグインの `default_method` なので、
推奨どおりで良ければそのまま Enter で確定できる。
...
| 変数 | 意味 |
|---|---|
| `$COCOON_INSTALL_METHOD` | 選ばれた method 名（例: `"binary"`）。プラグインが `[install.methods]` を宣言していれば常にセットされる。legacy の単一 `install.sh` プラグインでは空 |
