...
**pin / checksum のスコープ。** バージョン制約は `[plugins.versions]` 配下に
書かれ、**プラグイン単位（method 別ではない）** で保持される — catalog
の `plugin.toml` を method 別 checksum で肥大化させないという catalog
側のポリシー。per-arch checksum は `workspace.toml` ではなく `cocoon lock` が
`cocoon.lock` に記録する。method を切り替えるときは `cocoon lock` を再実行して
記録済み checksum を更新する。さもないと install スクリプトの
...
| `PIN`            | per-RUN env、`[version].version_capable = true` のときのみ | `workspace.toml` の `[plugins.versions]` セクション内 `<id> = "=..."` エントリの値（例: `=1.23.4` → `PIN="1.23.4"`）。lock 未生成の `latest` では空で、upstream 最新を使う |
...
| `<EXTRA_ENV>`    | per-RUN env、`[install.extra_versions]` で宣言されたときのみ | 宣言された subcomponent 版ごとに 1 つの env が出る。プラグイン側で env 名と default を宣言し、ユーザーは `workspace.toml` の `[plugins.versions].<id>` インラインテーブルに同じキー名を書いて値を上書きできる（例: `android-sdk = { version = "=...", api_level = "36" }`）。予約 env 名（上記）と衝突する env 名は宣言できない。詳細は後述の「サブコンポーネントバージョン」節を参照 |
...
1 プラグイン 1 文字列制約を記録する。checksum はここには記録せず、
...
`version_capable = false` のプラグインは制約を付けられない。`cocoon gen` は
...
- **キー名**（`api_level`, `build_tools`）— ユーザーが
  ...
  `version` は `[plugins.versions]` が消費する予約キー（inline-table
  形式でプラグイン自身のバージョン制約を担う）なので、extra キーとして
  宣言すると拒否される（宣言しても上書き不可能な no-op になるため）。
...
ユーザー側 workspace.toml では inline-table 形式を使い、プラグイン自身の
バージョンを予約キー `version` 配下に書く:
