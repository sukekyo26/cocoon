...
（例: `gh-cli`、`bun-sh`）は設定ファイルの `[plugins.methods]` を
...
| `[install]`  | `default_method`  | string             | —     | ✓ | 設定ファイルの `[plugins.methods]` で指定が無いときに採用される method 名。`[install.methods]` のキー名と一致する必要がある。`[install.methods]` 必須化のためこのフィールドも必須 (method 1 つだけのプラグインも 1 エントリ宣言する) |
...
インストール経路が選べるようになる。選択は設定ファイルに保存される:
...
設定ファイル側の旧参照が残っていても誤ったアーティファクトを引っ張る前
...
という catalog 側のポリシー。per-arch checksum は設定ファイルではなく
...
| `PIN`            | per-RUN env、`[version].version_capable = true` のときのみ | 設定ファイルの `[plugins].enable` 配列の `<id>=<version>` 要素の値（例: `"go=1.23.4"` → `PIN="1.23.4"`）。未 pin の id や lock 未生成の `latest` 要素では空で、upstream 最新を使う |
...
ユーザー側設定ファイルでは `[plugins.options]` インラインテーブルの
...
- [`architecture.ja.md`](architecture.ja.md) — LayeredFS と inline-heredoc
  設計の why。
- [`configuration.ja.md`](configuration.ja.md) — 設定ファイルの
