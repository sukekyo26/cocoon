...
env の値はダブルクォートのセマンティクスで出力します (安全文字のみの値はクォート無し、それ以外は `"..."` で囲み `$` はそのまま残す) ため、rc を source する時点で `$HOME` / `$PATH` などがシェルにより展開されます。`$(cmd)` コマンド置換は bash/zsh と fish 3.4+ (cocoon がサポートするベースイメージに同梱されているバージョン) で動作します。それ以前の fish では fish 固有の `(cmd)` 記法を使ってください。リテラルの `$` は `\$` と書けます。alias の本文はシングルクォートで出力します。alias は呼び出し時にシェルが本文を再パースするので、`$1` や `$HOME` を含めても呼び出し時に正しく解釈されます。
