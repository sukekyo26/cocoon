...
`[container.sudo]` の password モードは `cocoon init` が `cocoon.toml` 以外に書き込む唯一のケースです。`cocoon init --sudo password`（対話）は gitignore 対象の `SUDO_PASSWORD` シークレットを `.devcontainer/.env.local` に seed し、対応する無視ルールを `.devcontainer/.gitignore` に upsert します（既存ルールとファイルの権限は保持）。シークレットはビルド時に Docker build secret として消費され、コミットされず、`cocoon gen` でも再生成されません。
...
| `"../.."`, … | cwd から N 段上の祖先 | `/home/$USER/<dir>` | ホスト上で数段深くネストしたプロジェクトで、上位の祖先（とその兄弟）を見せたい場合 |

受け付けるのは `"."` か `".."` のみの連結だけで、生成される bind mount は常にプロジェクトディレクトリの祖先に解決されます。親マウントは選んだ祖先を `/home/$USER/<dir>` にフラットに対応させます（`<service>` 配下にネストするのは `"."` のときだけ）。
