...
- 生成される Dockerfile の `apt-get install` で同じ apt パッケージが複数回
  列挙される問題を修正しました。ベース・ログインシェル・プラグイン依存・
  `[apt].packages` の各セクションを横断して重複排除し、複数セクションが提供する
  パッケージは最初の出現を残して 1 回だけインストールするようにしました。
...
- `codex` プラグインが `bubblewrap` をインストールするようになりました。Codex は
  同梱版にフォールバックせず OS の `bwrap` でコマンドサンドボックスを実行し、
  「bubblewrap が PATH に見つからない」という警告も出なくなります。
