...
- `cocoon plugin pin` に `--method <name>` フラグを追加: pin は `[plugins.versions]` のインラインテーブル行に加えて `[plugins.methods]` 側の `<id> = "<method>"` 行も出力（`--write` 時は単一 read-modify-write で in-place upsert なので、途中失敗で workspace.toml が半端な状態にならない）します。指定された method がプラグインの `plugin.toml` に宣言されているか検証し、未宣言だった場合は宣言済み method 名一覧をエラーに含めて出すので typo は即座に直せます。checksum (`--amd64-checksum` / `--arm64-checksum`) はワークスペーススコープのまま (method 別ではない) なので、method を切り替えるときは新しいアーティファクトに合う SHA256 を渡し直してください。
- `copilot-cli` プラグインに 2 つ目のインストール方式 **`binary`** を追加（新しい既定 method は **`installer`**: 旧 `install.sh` だった gh.io インストーラ経路を改名）。`binary` は `github/copilot-cli` の GitHub Release から `copilot-linux-${arch}.tar.gz` を直接ダウンロードし、`~/.local/bin/copilot` に展開します。`gh.io` が遮断されている（Zscaler 等）環境や `curl|sh` を社内ポリシーが禁止する環境で利用します。`cocoon init --plugin-methods="copilot-cli=binary"`（または対話ピッカー）で切替可能 — 切替後は新しいアーティファクトに合う `checksum_amd64` / `checksum_arm64` を `[plugins.versions]` 側で更新してください（install スクリプトの `sha256sum -c -` 検証が走るため）。
...
- **BREAKING**: `copilot-cli` の method 名 `gh-cli` を **`installer`** に改名 (旧名は vendor 固有の語彙で、catalog 全体の命名規約から外れていたため)。`workspace.toml` に `[plugins.methods] copilot-cli = "gh-cli"` を書いていたユーザーは `"installer"` に書き換える必要があります。`cocoon plugin pin --method` は新名で検証し、エラーメッセージに宣言済み method 名一覧を出します。
