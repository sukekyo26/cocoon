...
- `[container].image` の候補に `dart` ベースイメージ (DockerHub `library/dart`、Debian trixie ベース) を追加しました。推奨タグは `stable` / `3.12.0` / `3.11.6` で、デフォルトは `stable`。Dart 公式が `stable` を現行安定版のローリングエイリアスとして明示推奨しているため、cocoon の curated 候補で **デフォルトがローリングタグになる初の image** です。`image = "dart"` と `[plugins].enable = ["dart"]` の併用は validation で reject、`cocoon init` でも `image = "dart"` 選択時に `dart` プラグインが picker から非表示になります (`golang` / `rust` / `node` / `denoland/deno` で既に動いている衝突パターンと同一)。`image = "dart"` と `flutter` プラグインの併用は意図的に許容しています — Flutter は `/usr/local/flutter/bin/cache/dart-sdk/bin` に自前 Dart を持ち、ベースイメージのツールチェーンを覆い隠さないためです。
