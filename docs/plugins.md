...
`cocoon.toml`. The rest of this page is for people writing or
modifying a plugin.
...
convention so the cocoon.toml `[plugins.methods]` table reads the
...
| `[install]`  | `default_method`  | string             | —     | ✓ | Name of the method picked when `cocoon.toml`'s `[plugins.methods]` does not pin one. Must be a key under `[install.methods]`. Required because `[install.methods]` is now mandatory (single-method plugins still declare one entry). |
...
so a stale `cocoon.toml` reference is caught before the script pulls
...
in `cocoon.lock` by `cocoon lock`, not in `cocoon.toml`. When switching
...
| `PIN`            | per-RUN env, only when `[version].version_capable = true` | Version string from the `<id>=<version>` element in `cocoon.toml`'s `[plugins].enable` array (e.g. `"go=1.23.4"` → `PIN="1.23.4"`). Empty for an unpinned id or a `latest` element with no lock yet, meaning "use upstream latest". |
...
- **`default`** — required, non-empty. Used when `cocoon.toml` does
...
- [`configuration.md`](configuration.md) — the `[plugins]`,
  `[plugins.methods]`, and `[plugins.options]` sections of `cocoon.toml`.
