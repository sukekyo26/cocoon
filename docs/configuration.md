...
`cocoon gen` walks upward from the current directory looking for a config file. `cocoon.toml` is preferred; `workspace.toml` is accepted as a fallback so existing projects keep working without a rename. At each directory it checks, in order:
...
It then ascends to the parent directory and repeats, stopping at a `.git` boundary or `$HOME`. The first match wins — a closer directory beats the preferred name in a parent (a child's `workspace.toml` outranks a parent's `cocoon.toml`). When both names exist in the same directory, `cocoon.toml` wins. Pass `--workspace <path>` to `cocoon gen` to override discovery.
...
`android-sdk`, `android-studio`, `flutter`, `zig` — see [`cocoon lock`](commands.md#sourceless-plugins));
